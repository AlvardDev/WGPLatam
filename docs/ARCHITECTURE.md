...
- **Alta de vendedor** (Fase 4): `auth.admin.inviteUserByEmail(email, { data: { role: 'seller', store_id } })` desde `lib/supabase/admin.ts` — como el `app_metadata` ya viene con rol y tienda, el trigger crea el perfil correctamente desde el primer INSERT, sin paso manual.
