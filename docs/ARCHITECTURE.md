...
- **No hay alta pública de vendedor**: se probó un auto-registro público (Fase 9) y se revirtió el mismo día por decisión explícita del usuario — ver `docs/PROGRESS.md`, "Reversión del auto-registro de vendedores". La única forma de que exista una cuenta de vendedor es la invitación de arriba; un correo que el admin no dio de alta no tiene fila en `auth.users` y `signInWithPassword` lo rechaza directo. El restablecimiento de contraseña de un vendedor ya activo sigue sin depender de correo: `/recuperar-vendedor` (público, solo correo) deja constancia en `seller_password_reset_requests`; el admin confirma identidad por fuera del sistema y aplica la contraseña nueva directo desde `/admin/vendedores/pendientes` — nunca se guarda en ninguna tabla.
