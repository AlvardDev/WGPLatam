...
- **Alta de vendedor por auto-registro** (Fase 9, `lib/actions/registro.ts`): segundo camino además de la invitación de arriba, pensado para no depender del cupo de correo del proveedor de email incluido de Supabase. El vendedor se registra solo en `/registro` (elige su propia contraseña) vía `auth.admin.createUser(..., { email_confirm: true })` — sin correo de confirmación. Igual que con la invitación, el trigger deja el perfil sin aprovisionar (`role = null`); un admin lo aprueba desde `/admin/vendedores/pendientes` (reusa `admin_finalize_seller_profile`) o lo rechaza (`deleteUser`, cascada a `profiles`). Mientras espera, `proxy.ts` lo manda a `/pendiente` en vez de dejarlo varado (cualquier sesión con claims pero `role = null` aterriza ahí, antes que cualquier otra regla del proxy). El restablecimiento de contraseña de un vendedor ya activo sigue el mismo espíritu: `/recuperar-vendedor` (público, solo correo) deja constancia en `seller_password_reset_requests` en vez de mandar un correo; el admin confirma identidad por fuera del sistema y aplica la contraseña nueva directo desde el módulo — nunca se guarda en ninguna tabla. Ver `docs/PROGRESS.md`, "Auto-registro de vendedores + aprobación manual", para el detalle completo.
