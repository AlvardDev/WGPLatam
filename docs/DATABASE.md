...
|---|---|---|
| `profiles` | `id` (= `auth.users.id`), `full_name`, `role` (`admin`\|`seller`), `store_id`, `is_active` | CHECK: seller ⇒ `store_id` not null; admin ⇒ null. Creado por trigger sobre `auth.users` desde `raw_app_meta_data`; para un vendedor invitado (Fase 4) se crea sin rol y lo completa `admin_finalize_seller_profile` — ver "RPC críticos" |
