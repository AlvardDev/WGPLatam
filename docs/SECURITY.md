...
| Registro público de vendedor (`/registro`, sin correo, Fase 9) sin límite por IP | **Aceptado, no mitigado todavía**: aprobación manual de admin es el filtro real (un registro no aprobado no tiene ningún acceso), pero alguien podría scriptear registros falsos y llenar el módulo de pendientes. Sistema todavía en localhost sin desplegar — resolver antes de exponerlo públicamente (throttle por IP, mismo patrón que `lookup_serial_attempts` pero sin `auth.uid()`). |
...
**Fase 8**: `private.is_admin()` reemplazada para exigir `aal2` además del rol — no es una RPC nueva, es el checklist aplicándose retroactivamente a las ~25 funciones que ya la usaban (F2-F7): todas quedan protegidas por el mismo cambio, sin editar sus archivos. `lookup_serial` reemplazada para añadir el throttle (`lookup_serial_attempts`, sin política RLS ni GRANT directo — inalcanzable desde la API salvo por la propia función `SECURITY DEFINER`, mismo patrón que `audit_logs` para `UPDATE`/`DELETE`). Como consecuencia del cambio en `is_admin()`, las fixtures de admin de `02` a `11` (que ya estaban escritas y confirmadas contra el proyecto real en F2-F7) se actualizaron para incluir `"aal":"aal2"` en su JWT simulado — sin eso, is_admin() empezaría a rechazar esas mismas pruebas que hoy pasan; no es una reescritura de esas fases, es la fixture reflejando la nueva realidad de una función compartida. Tests pgTAP contra el proyecto real (SQL Editor del dashboard, autorización explícita del usuario — mismo método que F2-F7): `12_mfa_and_throttle.sql`, nuevo — **8/8** (`is_admin()` en aal1/aal2/vendedor-con-aal2 llamada directamente, una integración real vía RLS de `audit_logs` con 0 filas en aal1 y filas visibles en aal2 — no solo la función aislada —, y el throttle: 30 llamadas dentro del límite vía un loop explícito, la 31 rechazada, un segundo usuario con su propio contador); `11_claims_and_reports.sql` (F7, re-verificado con las fixtures de aal2) — **55/55**, elegido como el archivo con más superficie dependiente de `is_admin()` para confirmar que el cambio compartido no rompió nada. Las fixtures de `02` a `10` recibieron el mismo cambio mecánico y, en una sesión posterior (2026-09-16), se re-corrieron individualmente contra el proyecto real con la misma técnica — resultado: **232/232**, ningún fallo atribuible a `is_admin()`/aal2 (dos hallazgos reales no relacionados, uno de test y uno de producto, encontrados y corregidos en el proceso; detalle completo en `docs/PROGRESS.md`, checkpoint F8). La verificación de esta fase queda así completa sobre los 11 archivos de pgTAP existentes (`02`-`12`), no solo sobre la muestra representativa (`11`, `12`) con la que se cerró originalmente.

**Fase 9 (rol superadmin, 2026-09-18)**: `admin_finalize_admin_profile`, `admin_set_admin_active` — mismo checklist que Fase 4 (`search_path=''`, revoke explícito de `anon`/`public`, re-verificación de rol dentro de la función), pero gated por `private.is_superadmin()` (nueva, exclusiva de `role='superadmin'`) en vez de `private.is_admin()` — un admin normal no puede invocarlas. `private.is_admin()` se extendió para aceptar `role in ('admin','superadmin')`, heredando de una sola vez toda la RLS/RPC ya escrita en F2-F9 sin tocar esos archivos (mismo patrón que el `aal2` de F8). `admin_set_admin_active` rechaza explícitamente un target que no sea `role='admin'` — ni siquiera un superadmin puede desactivar a otro superadmin o a sí mismo por esta vía, para no bloquear el único acceso sin pasar por el bootstrap manual (`docs/ARCHITECTURE.md`, "Primer admin"). Bug real encontrado por el propio pgTAP de esta migración antes de cerrarla: `private.handle_new_user()` (Fase 1) tiene su propia allow-list de roles válidos en `app_metadata`, independiente del `CHECK` de la tabla — no se había extendido, así que crear cualquier superadmin (incluido el bootstrap del primero) fallaba con "invalid role in app_metadata: superadmin"; corregido en la misma migración. Tests pgTAP: **20/20** (`13_superadmin.sql`) — is_admin()/is_superadmin() para los 3 roles, las 2 RPC como admin normal (rechazadas) y como superadmin (validaciones + éxito + no-doble-provisioning + no-auto-desactivación), constraint `profiles_role_store_shape` rechazando `store_id` en un superadmin. Regresión verificada corriendo `08_stores_and_sellers.sql` de nuevo tras el cambio: sigue en 21/21.
